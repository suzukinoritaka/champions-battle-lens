...
- 色違い・フォルム・メガ候補を含む718画像・359種/フォームの照合
- 低信頼時だけOpenAI画像認識へ切り替える任意のAI補助
- 低信頼またはAI利用不可のときは誤った6体を確定せず「認識できません」と表示
...
- 現在の端末内認識対象は、Championsメニュースプライトとして公開されている全718画像（通常色359＋色違い359）です。
- 端末負荷を抑えるため、認識画像は8枚のアトラスへまとめています。
- 認識後の表示アイコンも同じアトラスから切り出すため、外部画像サイトの読込可否に依存しません。
