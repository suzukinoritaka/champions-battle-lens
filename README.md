...
Pokémon Champions の見せ合い画面を選び、自分と相手の12体を読み取って、ダメージ・素早さ・タイプ相性を比較する iPhone 向けPWA試作です。
...
- 見せ合い画面のスクリーンショット選択
- Championsのメニュースプライトを使った端末内の自分6体＋相手6体認識
- 自分の6体が登録パーティと一致した場合は、登録済みの技・配分・持ち物を自動使用
- 未登録パーティの場合は、認識した6体のタイプ・標準ステータス・一般技で自動作成
- 自分側の信頼度が低い場合は誤ったパーティを確定せず、選択中の登録パーティを使用
...
- 見せ合い画面を左右12枠に分け、登録済みChampionsアイコンと端末内で照合します。
...
- 実スクリーンショット4枚・左右48枠で48枠一致を確認しています。
